...
def main(args: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--version", "-v",
        action="version",
        version=f"timetrace {__version__}",
    )
...
def _cmd_diff(
    ...
    from timetrace.diff import diff_cassettes, format_diff_report
    from timetrace.exceptions import CassetteNotFoundError, CassetteSchemaError
...
def _cmd_timeline(
    ...
    from timetrace.exceptions import CassetteNotFoundError, CassetteSchemaError
    from timetrace.timeline import generate_timeline, render_timeline_html
...
def _cmd_s3(parsed) -> int:
    ...
    bucket = getattr(parsed, 'bucket', None) or os.environ.get("TIMETRACE_S3_BUCKET")
    if not bucket:
...
def _cmd_search(parsed) -> int:
    ...
    from timetrace.catalog import search_cassettes
...
def _cmd_index(parsed) -> int:
    ...
    from timetrace.catalog import build_index, save_index
