"""
S3 storage backend for Timetrace cassettes.
...
class S3Config:
    ...
    def from_env(cls) -> "S3Config":
        ...
        """
        bucket = os.environ.get("TIMETRACE_S3_BUCKET")
        if not bucket:
            raise ValueError("TIMETRACE_S3_BUCKET environment variable is required")

        return cls(
            bucket=bucket,
            prefix=os.environ.get("TIMETRACE_S3_PREFIX", "cassettes"),
            region=os.environ.get("TIMETRACE_S3_REGION") or os.environ.get("AWS_REGION"),
            endpoint_url=os.environ.get("TIMETRACE_S3_ENDPOINT"),
...
class S3Store:
    ...
    Usage:
        from timetrace.storage.s3 import S3Store, S3Config
    ...
    def _create_client(self) -> "S3Client":
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install timetrace[s3]"
            )
