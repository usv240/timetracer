"""
Flask integration for Timetrace.
...
from timetrace.cassette import read_cassette, write_cassette
from timetrace.config import TraceConfig
from timetrace.context import reset_session, set_session
from timetrace.policies import redact_body, redact_headers
from timetrace.session import ReplaySession, TraceSession
from timetrace.types import BodySnapshot, RequestSnapshot, ResponseSnapshot
from timetrace.utils.hashing import hash_body
...
class TimeTraceMiddleware:
    """
    WSGI middleware for Timetrace integration with Flask.
    ...
    def __init__(
        ...
        Args:
            ...
            config: Timetrace configuration. If None, loads from environment.
        """
        self.app = app
    ...
    def _handle_replay(
        ...
        if not cassette_path:
            print("TIMETRACE [WARN] replay mode requires TIMETRACE_CASSETTE", file=sys.stderr)
            return self.app(environ, start_response)
    ...
    def _build_response_snapshot(
        ...
        from timetrace.policies import should_store_body
    ...
    def _print_record_summary(self, session: TraceSession, cassette_path: str) -> None:
        ...
        print(
            f"TIMETRACE {icon} recorded {method} {path}  "
    ...
    def _print_replay_summary(self, session: ReplaySession, duration_ms: float) -> None:
        ...
        print(
            f"TIMETRACE replay {method} {path}  "
...
def init_app(app: "Flask", config: TraceConfig | None = None) -> None:
    """
    Initialize Timetrace for a Flask app.

    Alternative to wrapping wsgi_app directly.

    Args:
        app: Flask application instance.
        config: Timetrace configuration.
